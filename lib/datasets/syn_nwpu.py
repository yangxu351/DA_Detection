...
class syn_nwpu(imdb):
    def __init__(self, image_set='syn_nwpu_c1_train', devkit_path=None):
        imdb.__init__(self, image_set)
        cix = image_set.rfind('_')
        data_cat = image_set[:cix].upper() # 'syn_nwpu_c1'
        
        database = cfg_d.DATABASE # 'syn*C1_v*'

        self._year = '2022'
        self._image_set = image_set[cix+1:].lower() # train/val
        self._devkit_path = os.path.join(cfg_d.DEV_DATA_DIR, data_cat, database) #self._get_default_path() if devkit_path is None \
            #else devkit_path
        self._data_path = os.path.join(cfg_d.BASE_DATA_DIR, data_cat, database)
        print('data path', self._data_path)
        self._classes = ('NWPU_C0',  # always index 0
    ...
    def _load_image_label_arr(self):
        """
        Load the dataset's image and label arr.
        """
        # Example path to image set file:
        # self._devkit_path + /Main/val.txt
        img_set_file = self.get_img_set_file()
        lbl_set_file = self.get_lbl_set_file()
        arr_img = pd.read_csv(img_set_file, header=None).squeeze().to_numpy()
        arr_lbl = pd.read_csv(lbl_set_file, header=None).squeeze().to_numpy()
        assert arr_img.shape==arr_lbl.shape, 'img shape not equal to lbl shape.'
        return arr_img, arr_lbl
    ...
    def image_id_at(self, i):
        """
        Return  image index i in the image sequence.
        """
        return i
...
if __name__ == '__main__':
    ...
    d = syn_nwpu('syn_nwpu_c1_train', database)
    # d = syn_nwpu('syn_nwpu_c1_val', database)
    res = d.roidb
