...
# tag: yang add 
from lib.model.solver.build import make_lr_scheduler, make_optimizer
...
if __name__ == '__main__':
    ...
    # tag: yang added
    optimizer = make_optimizer(cfg, fasterRCNN)
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            if step % args.disp_interval == 0:
                ...
                print("\t\t\tdloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f eta: %.4f"\
                    % (dloss_s, dloss_t, dloss_s_p, dloss_t_p, args.eta))
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
